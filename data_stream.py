...
def run_spark_job(spark):
    ...
    agg_df = distinct_table.withWatermark("call_date_time", "60 minutes") \
        .groupBy("original_crime_type_name", "disposition", psf.window("call_date_time", "30 minutes")).count()
    agg_df.printSchema()
    ...
    query = agg_df.writeStream.queryName("query_agg_df").format("console").outputMode("complete").option("numRows", 30).start()
